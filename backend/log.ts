...
class Logger {
    ...
    /**
     * REDAIRANGE_HIDE_LOG=debug_monitor,info_monitor
     *
     * Example:
     *  [
     ...
     */
    constructor() {
        if (typeof process !== "undefined" && process.env.REDAIRANGE_HIDE_LOG) {
            const list = process.env.REDAIRANGE_HIDE_LOG.split(",").map(v => v.toLowerCase());
            ...
            this.debug("server", "REDAIRANGE_HIDE_LOG is set");
