...
/**
 * redairange Instance Manager
 * One AgentManager per Socket connection
 */
export class AgentManager {

    protected socket : RedAiRangeSocket;
    ...
    constructor(socket: RedAiRangeSocket) {
        this.socket = socket;
    }
    ...
    test(url : string, username : string, password : string) : Promise<void> {
        return new Promise((resolve, reject) => {
            ...
            if (!endpoint) {
                reject(new Error("Invalid redairange URL"));
            }

            if (this.agentSocketList[endpoint]) {
                reject(new Error("The redairange URL already exists"));
            }
            ...
            client.on("connect_error", (err) => {
                if (err.message === "xhr poll error") {
                    reject(new Error("Unable to connect to the redairange instance"));
                } else {
                    reject(err);
                }
                client.disconnect();
