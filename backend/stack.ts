...
import { RedAiRangeSocket, fileExists, ValidationError } from "./util-server";
...
export class Stack {
    ...
    protected server: RedAiRangeServer;

    protected combinedTerminal? : Terminal;

    protected static managedStackList: Map<string, Stack> = new Map();

    constructor(server : RedAiRangeServer, name : string, composeYAML? : string, composeENV? : string, skipFSOperations = false) {
        this.name = name;
        this.server = server;
    ...
    toSimpleJSON(endpoint : string) : object {
        return {
            ...
            status: this._status,
            tags: [],
            isManagedByRedAiRange: this.isManagedByRedAiRange,
    ...
    get isManagedByRedAiRange() : boolean {
    ...
    async deploy(socket : RedAiRangeSocket) : Promise<number> {
    ...
    async delete(socket: RedAiRangeSocket) : Promise<number> {
        const terminalName = getComposeTerminalName(socket.endpoint, this.name);
    ...
    static async getStackList(server : RedAiRangeServer, useCacheForManaged = false) : Promise<Map<string, Stack>> {
        ...
        for (let composeStack of composeList) {
            ...
            // This stack probably is not managed by redairange, but we still want to show it
            if (!stack) {
                // Skip the redairange stack if it is not managed by redairange
                if (composeStack.Name === "redairange") {
                    continue;
                }
    ...
    static async getStack(server: RedAiRangeServer, stackName: string, skipFSOperations = false) : Promise<Stack> {
    ...
    async start(socket: RedAiRangeSocket) {
        const terminalName = getComposeTerminalName(socket.endpoint, this.name);
        let exitCode = await Terminal.exec(this.server, socket, terminalName, "docker", [ "compose", "up", "-d", "--remove-orphans" ], this.path);
        if (exitCode !== 0) {
    ...
    async stop(socket: RedAiRangeSocket) : Promise<number> {
        const terminalName = getComposeTerminalName(socket.endpoint, this.name);
    ...
    async restart(socket: RedAiRangeSocket) : Promise<number> {
        const terminalName = getComposeTerminalName(socket.endpoint, this.name);
    ...
    async down(socket: RedAiRangeSocket) : Promise<number> {
        const terminalName = getComposeTerminalName(socket.endpoint, this.name);
    ...
    async update(socket: RedAiRangeSocket) {
        const terminalName = getComposeTerminalName(socket.endpoint, this.name);
    ...
    async joinCombinedTerminal(socket: RedAiRangeSocket) {
    ...
    async leaveCombinedTerminal(socket: RedAiRangeSocket) {
        const terminalName = getCombinedTerminalName(socket.endpoint, this.name);
    ...
    async joinContainerTerminal(socket: RedAiRangeSocket, serviceName: string, shell : string = "sh", index: number = 0) {
