...
import { Arguments, Config, RedAiRangeSocket } from "./util-server";
...
export class RedAiRangeServer {
    ...
    constructor() {
        ...
        let unexpectedErrorHandler = (error : unknown) => {
            console.trace(error);
            console.error("If you keep encountering errors, please report to https://github.com/ErdemOzgen/redairange");
        };
        ...
        this.config.sslKey = args.sslKey || process.env.REDAIRANGE_SSL_KEY || undefined;
        this.config.sslCert = args.sslCert || process.env.REDAIRANGE_SSL_CERT || undefined;
        this.config.sslKeyPassphrase = args.sslKeyPassphrase || process.env.REDAIRANGE_SSL_KEY_PASSPHRASE || undefined;
        this.config.port = args.port || Number(process.env.REDAIRANGE_PORT) || 5002; // TODO: Change to 5001 5002 in dev port: 5002 in docker push : 5001
        this.config.hostname = args.hostname || process.env.REDAIRANGE_HOSTNAME || undefined;
        this.config.dataDir = args.dataDir || process.env.REDAIRANGE_DATA_DIR || "./data/";
        this.config.stacksDir = args.stacksDir || process.env.REDAIRANGE_STACKS_DIR || defaultStacksDir;
        ...
        this.io.on("connection", async (socket: Socket) => {
            let redairangeSocket = socket as RedAiRangeSocket;
            redairangeSocket.instanceManager = new AgentManager(redairangeSocket);
            redairangeSocket.emitAgent = (event : string, ...args : unknown[]) => {
                let obj = args[0];
                if (typeof(obj) === "object") {
                    let obj2 = obj as LooseObject;
                    obj2.endpoint = redairangeSocket.endpoint;
                }
                redairangeSocket.emit("agent", event, ...args);
            };

            if (typeof(socket.request.headers.endpoint) === "string") {
                redairangeSocket.endpoint = socket.request.headers.endpoint;
            } else {
                redairangeSocket.endpoint = "";
            }

            if (redairangeSocket.endpoint) {
                log.info("server", "Socket connected (agent), as endpoint " + redairangeSocket.endpoint);
            } else {
                log.info("server", "Socket connected (direct)");
            }

            this.sendInfo(redairangeSocket, true);

            if (this.needSetup) {
                log.info("server", "Redirect to setup page");
                redairangeSocket.emit("setup");
            }

            // Create socket handlers (original, no agent support)
            for (const socketHandler of this.socketHandlerList) {
                socketHandler.create(redairangeSocket, this);
            }
            ...
            for (const socketHandler of this.agentSocketHandlerList) {
                socketHandler.create(redairangeSocket, this, agentSocket);
            }

            // Create agent proxy socket handlers
            this.agentProxySocketHandler.create2(redairangeSocket, this, agentSocket);
            ...
            if (await Settings.get("disableAuth")) {
                ...
                this.afterLogin(redairangeSocket, await R.findOne("user") as User);
                redairangeSocket.emit("autoLogin");
            } else {
                log.debug("auth", "need auth");
            }

            // Socket disconnect
            redairangeSocket.on("disconnect", () => {
                log.info("server", "Socket disconnected!");
                redairangeSocket.instanceManager.disconnectAll();
            });
    ...
    async afterLogin(socket : RedAiRangeSocket, user : User) {
        ...
        // Also connect to other redairange instances
    ...
    async serve() {
        ...
        // If there is no record in user table, it is a new redairange instance, need to setup
        if (userCount == 0) {
    ...
    async sendInfo(socket : Socket, hideVersion = false) {
        ...
        if (!hideVersion) {
            ...
            isContainer = (process.env.REDAIRANGE_IS_CONTAINER === "1");
        }
        ...
        for (let socket of socketList) {
            ...
            if (redairangeSocket.userID) {
                ...
                for (let [ stackName, stack ] of stackList) {
                    map.set(stackName, stack.toSimpleJSON(redairangeSocket.endpoint));
                }

                log.debug("server", "Send stack list to user: " + redairangeSocket.id + " (" + redairangeSocket.endpoint + ")");
                redairangeSocket.emitAgent("stackList", {
                    ok: true,
    ...
    disconnectAllSocketClients(userID: number | undefined, currentSocketID? : string) {
        for (const rawSocket of this.io.sockets.sockets.values()) {
            let socket = rawSocket as RedAiRangeSocket;
