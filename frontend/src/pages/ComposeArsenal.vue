<template>
    <transition name="slide-fade" appear>
        <div>
            <h1 v-if="isAdd" class="mb-3">{{ $t("Arsenal Tool Deploy") }}</h1>
  ...
  services:
    nginx:
      ...
      ports:
        - "8080:80"
  `;
const envDefault = "# VARIABLE=value #comment # Arsenal Tools";
...
export default {
    ...
    computed: {
        // Computed property: add stackname to  _arsenal
        targetStackName() {
            if (!this.stack.name) {
                return "";
            }
            return this.stack.name.endsWith("_arsenal")
                ? this.stack.name
                : this.stack.name + "_arsenal";
        },
        endpointDisplay() {
        ...
        isAdd() {
            return this.$route.path === "/composearsenal" && !this.submitted;
        },
        globalStack() {
    ...
    methods: {
        ...
        // make lowercase and add _arsenal to end
        stackNameToLowercase() {
            if (!this.stack.name) {
                return;
            }
            this.stack.name = this.stack.name.toLowerCase();
            if (!this.stack.name.endsWith("_arsenal")) {
                this.stack.name += "_arsenal";
            }
        },
        deployStack() {
